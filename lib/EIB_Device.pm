...
=item C<new>
Generic EIB_Device class. This class is not instantiated directly. 
It is used to inherit common device properties.

Instance variables:
  outqueue  Queue (list) of messages to be sent
  passes    Number of passes since last message sent. For limiting sending rate
=cut
...
sub new {
    ...
    $self->{outqueue} = [];    # Queue of messages to be sent
    $self->{passes}   = 0;     # Number of passes since last message sent
...
"Parameter eib_device has changed to eib_connection!\nPlease change ini file!\n\n"
    ...
    if ( $dev =~ /knxiprouter/ ) {
        $EIB_Device = EIB_Device_KNXIProuter->new();
    }
    elsif ( $dev =~ /(.+):(.+)/ ) {
...
package EIB_Device_KNXIProuter;

#
@EIB_Device_KNXIProuter::ISA = ('EIB_Device');

#@EIB_Device_KNXIProuter::ISA = ('Generic_Item');

=item C<new>
Device for communication with EIB/KNX using KNXnet/IP over UDP multicast. 
Inherits EIB_Device.

Instance variables:
  port  	UDP multicast port for sending and receiving (eib_knxiprouter_port, default 3671)
  multicast     UDP multicast address for sending and receiving (eib_knxiprouter_multicast, default 224.0.23.12)
  interface	Interface to use (eib_knxiprouter_interface, default undef = all interfaces)
  physaddr	Physical (individual) EIB/KNX address to use as soure address for messages
                generated by us (eib_knxiprouter_physaddr, default 0.0.244)
=cut
...
sub new {
    ...
    $self->{port}      = $::config_parms{eib_knxiprouter_port};
    $self->{multicast} = $::config_parms{eib_knxiprouter_multicast};
    $self->{interface} = $::config_parms{eib_knxiprouter_interface};
    $self->{physaddr}  = $::config_parms{eib_knxiprouter_physaddr};
    $self->{ipsocket} = undef;    # IP multicast socket
    $self->{count}    = 0;        # Number of passes since last message sent

    # Defaults
    $self->{port}      = 3671          unless ( defined $self->{port} );
    $self->{multicast} = '224.0.23.12' unless ( defined $self->{multicast} );
    $self->{physaddr}  = '0.0.244'     unless ( defined $self->{physaddr} );

    print "Created KNXIProuter device:\n";
    foreach ( keys %{$self} ) {
        print "$_", ": ", $self->{$_}, "\n";
    }
    $self->openIPSocket();
    &::MainLoop_pre_add_hook(
        sub { EIB_Device_KNXIProuter::check_for_data($self); }, 1 );
    return $self;
}
...
sub openIPSocket {
    ...
    # Uses IP multicast. On Debian, install with
    # "apt-get install libio-socket-multicast-perl"
    eval "use IO::Socket::Multicast";
    if ($@) {
        &main::print_log(
            "KNXIProuter: Error loading module IO::Socket::Multicast");
        return;
    }

    # create a new UDP socket ready to read datagrams on port
    my $s;
    unless ( $s = IO::Socket::Multicast->new( LocalPort => $self->{port} ) ) {
        &main::print_log(
            "KNXIProuter: Can't open multicast port $self->{port}");
        return;
    }

    # Add a multicast group
    unless ( $s->mcast_add( $self->{multicast}, $self->{interface} ) ) {
        &main::print_log(
"KNXIProuter: Can't add multicast group $self->{multicast} $self->{interface}"
        );
        close($s);
        return;
    }

    # Set destination for multicast
    $s->mcast_dest("$self->{multicast}:$self->{port}");

    $self->{ipsocket} = $s;
    return $self->{ipsocket};
}
...
sub input_msg {
    ...
    $self->openIPSocket() unless $self->{ipsocket};

    if ( $self->{ipsocket} ) {
        $rin = $win = $ein = '';
        vec( $rin, fileno( $self->{ipsocket} ), 1 ) = 1;
        $ein = $rin;
        ($nfound) = select( $rout = $rin, undef, undef, 0 );
        if ( $nfound > 0 ) {
            my $whence = $self->{ipsocket}->recv( $buf, 1024 );
            return $self->decode_cemi($buf);
        }
    }
}
...
sub output_msg {
    ...
    # Need a valid source address for ROUTING INDICATIOn message
    $mref->{src} = $self->{physaddr};

    if ( my $msg = ( encode_cemi($mref) ) ) {
        if ( $self->{ipsocket} ) {
            $self->{ipsocket}->mcast_send($msg)
              or &main::print_log(
"KNXIProuter: Can't send EIB message to multicast group $self->{multicast} $self->{interface}"
              );
        }
    }
}

# decode_cemi: unmarshall a string with an EIB message in a cEMI frame into a hash.
# This is highly partial decoding: we recognize a ROUTING_INDICATION frame, but nothing else.
#
# The resulting hash has the follwing fields:
#	- type: APCI (symbolic value)
#	- src: source address
#	- dst: destiniation address
#	- data: array of integers; one for each byte of data
#
# The cEMI frame looks like this
# (from http://blog-en.openalfa.com/introduction-to-eibknx):
#+---------+--------+--------+--------+--------+---------+---------+--------+---------+
...
#+--------+--------+--------+--------+
#| Header |Version |Service | Total  |
#| length |(0x10)  |type    | length |
#+--------+--------+--------+--------+
...
sub decode_cemi {
    ...
    if ( length($buf) < 6 ) {
        &main::print_log( "decode_cemi: too short cEMI frame: ",
            length($buf), " bytes" );
        return undef;
    }

    my ( $h_hlen, $h_version, $h_stype, $h_totlen, $cemi_frame ) =
      unpack( "CCnna*", $buf );

    # Verify it is a valid cEMI frame header
    if ( $h_hlen != 6 || $h_version != 0x10 || $h_totlen != length($buf) ) {
        &main::print_log(
"decode_cemi: invalid cEMI header (hlen $h_hlen, version $h_version, totlen $h_totlen)"
        );
        return undef;
    }
    if ( $h_stype != 0x0530 ) {    # Not ROUTING_INDICATION
        &main::print_log(
            "decode_cemi: Not ROUTING INDICATION (service type $h_stype)");
        return undef;
    }

    # Proceed with frame
    my ( $m_code, $addlen, $ctrl1, $ctrl2, $src, $dst, $apdulen, $apdu ) =
      unpack( "CCCCnnCa*", $cemi_frame );

    if ( $main::config_parms{eib_errata} >= 3 ) {
        my $s = sprintf(
            "Received cEMI: hsize  $h_hlen, version 0x%x, stype 0x%x, len %d.",
            $h_hlen, $h_version, $h_stype, $h_totlen );
        $s = $s
          . sprintf( " mcode 0x%x, addlen %d, ctl1 0x%x ctl2 0x%x, apdulen %d",
            $m_code, $addlen, $ctrl1, $ctrl2, $apdulen );
        &main::print_log($s);
    }

    # Verify it is a valid L_Data.ind message
    if ( ( $m_code != 0x29 && $m_code != 0x11 ) || $addlen != 0 ) {
        &main::print_log(
"decode_cemi: not a L_Data.ind frame (code $m_code, additional info len $addlen)"
        );
        return undef;
    }

# Now deconstruct Application PDU. Some fiddling required to separate APCI and data.
# Byte 1 is 6 bits of TPCI and 2 most significant bits of APCI.
# Byte 2 is 2 least significant bits of APCI, the rest (the 6 least signficant bits) is application data:
# +--------+--------+--------+---
# |TTTTTTAA|AADDDDDD|DDDDDDDD|DDD
# +--------+--------+--------+---
# http://www.openremote.org/display/forums/CEMI+APDU+Field

    my ( $tpci_apci, $data ) = unpack( "Ca*", $apdu );

    my $apci = ( vec( $tpci_apci, 0, 2 ) << 2 ) | vec( $data, 3, 2 );
    ...
    $msg{'src'} = EIB_Device::addr2str($src);
    $msg{'dst'} = EIB_Device::addr2str( $dst, $ctrl2 >> 7 );

    @data = unpack( "C" . bytes::length($data), $data );
    $msg{'data'} = \@data;
    $msg{'buf'} = unpack( "H*", $buf ) if $main::config_parms{eib_errata} >= 4;
    return \%msg;
}
...
sub encode_eib {
    ...
    @msg  = (
        ...
        str2addr( $mref->{'dst'} ),    # Destination address
        0x60 | ( $#$data + 1 ) |       # Routing, length and
          ( EIB_Device::addrIsLogical( $mref->{'dst'} ) << 7 ),   # address type
        0x0 | ( $APCI >> 2 ),    # TPDU type, Sequence no, APCI (msb)
        ( ( $APCI & 0x3 ) << 6 ) | $$data[0],
    );
    if ( $#$data > 0 ) {
...
# encode: marshall a hash into an EIB message string
sub encode_cemi {
    my ($mref) = @_;
    my @msg;
    my $APCI;
    ...
    #header size 6, protoversion 0x10, service_type 0x530, totlen 19
    # ctrl1 0x9c, 0xbc
    @msg = (
        ...
        6,               # Header length
        0x10,            # KNXnet version
        0x0530,          # Service type (ROUTING INDICATION)
        17 + $#$data,    # Total length (cEMI header + cEMI frame + APDU + data)
                         # cEMI frame (9 bytes)
        0x29,            # Message code (L_Data.ind)
        0,               # Additional info length
        0xbc,            # ctrl1 (standard frame, repeat, broadcast, low prio)
        ( EIB_Device::addrIsLogical( $mref->{'dst'} ) << 7 ) |
          0x50,          # ctrl2 (individual/group, hop count 5)
        EIB_Device::str2addr( $mref->{'src'} ),    # Source address
        EIB_Device::str2addr( $mref->{'dst'} ),    # Destination address
        $#$data + 1,                               # APDU data length
                                                   # APDU (2+ bytes)
        0x0 | ( $APCI >> 2 ),                      # TPCI (zero), APCI (msb)
        ( ( $APCI & 0x3 ) << 6 ) | $$data[0],      # APCI (lsb), data
    );
    if ( $#$data > 0 ) {
        push @msg, @$data[ 1 .. $#$data ];
    }
    return ( pack "CCnnCCCCnnCCC" . ( $#msg - 11 ), @msg );
}
...
sub check_for_data {
    ...
    if ( my $msg = $self->input_msg() ) {
        EIB_Item::receive_msg($msg);
    }

    # Check for output
    $self->{count}++ unless $self->{count} > $::config_parms{eib_send_interval};
    if ( ( $#{ $self->{outqueue} } >= 0 )
        && $self->{count} >= $::config_parms{eib_send_interval} )
    {
        $self->{count} = 0;
        my $mref = shift @{ $self->{outqueue} };
        $self->output_msg($mref);
    }
}
